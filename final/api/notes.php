...
switch ($method) {
    ...
    case 'POST':
        ...
        $query = "INSERT INTO notes (user_id, title, content, category_id, tags, client_id, created_at, 
                    updated_at) 
                    VALUES (:user_id, :title, :content, :category_id, :tags, :client_id,:created_at, 
                    :updated_at)";
        $stmt = $db->prepare($query);
        $stmt->bindParam(":user_id", $user_id);
...
function softDeleteNote($db, $user_id, $note_id) {
    ...
    // 软删除：将is_deleted设置为1
    $query = "UPDATE notes SET category_id = 'deleted', is_deleted = 1,updated_at = datetime('now','localtime') WHERE id = :id AND user_id = :user_id";
    $stmt = $db->prepare($query);
    $stmt->bindParam(":id", $note_id);
    $stmt->bindParam(":user_id", $user_id);
...
function restoreNote($db, $user_id, $note_id) {
    ...
    $query = "UPDATE notes SET is_deleted = 0, category_id = :target_category, updated_at = datetime('now','localtime') WHERE id = :id AND user_id = :user_id";
    $stmt = $db->prepare($query);
    $stmt->bindParam(":id", $note_id);
    $stmt->bindParam(":user_id", $user_id);
