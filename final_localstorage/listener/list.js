...
const NotesListListener = {
        ...
        if(SidebarUI.currentCategoryId==='deleted'){
            ...
            if (confirm(`确定要删除笔记 "${note.title}" 吗？`)) {
                ...
                if (success) {
                    ...
                    // 如果删除的是当前选中的笔记，清空编辑器
                    if (noteId === this.notesListUI.getCurrentNoteId()) {
    ...
    performMoveNote() {
        ...
        // 调用数据层移动笔记
        const success = NoteDB.moveNoteToCategory(this.noteToMove, targetCategoryId);
    ...
    setPrivatePassword(modal) {
        ...
        // 设置密码
        const result = NoteDB.setPrivatePassword(password1);
    ...
    // 清除私密笔记密码
    clearPrivatePassword(modal) {
        const result = NoteDB.clearPrivatePassword();
